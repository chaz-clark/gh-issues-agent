...
2. **Sync issues**: Run `uv run tools/gh_sync.py` to pull all open issues + PRs + comments into `.github-issues/open/`
...
Files written by `gh_sync.py` to `.github-issues/open/`:
...
- [ ] `.github-issues/` folder exists (or will be created by `gh_sync.py`)
