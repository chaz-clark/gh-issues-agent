...
Open an issue file from `.github-issues/open/`, read the description + comments, decide whether to fix it.
...
The issue is created on GitHub, and the local mirror drops into that repo's `.github-issues/open/` (if you sync that repo).
