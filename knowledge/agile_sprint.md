...
| — | Establish `.github_issues/` local mirror pattern | S | `[x]` | Initial extraction |
...
- Local mirror syncs to `.github_issues/open/` and `closed/`
...
3. Run `gh_sync.py` to confirm all sprint issues are in `.github_issues/closed/` (if tracked on GitHub)
